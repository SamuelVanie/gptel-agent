...
Repeated similar/cyclic calls stop after =gptel-agent-max-loop-violations=, and
web-heavy sub-agents have a separate =gptel-agent-max-web-tool-calls= budget.
Safety stops and round limits are returned as explicit inconclusive reports
with the attempts made, reason and confidence; they are not treated as a
requirement to manufacture a positive answer.  Web searches default to
