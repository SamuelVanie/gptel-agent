...
Repeated similar/cyclic calls are blocked and returned to the model as
actionable errors; they do not terminate the sub-agent.  After
=gptel-agent-max-loop-violations= the feedback explicitly directs the model to
report with the evidence it has.  Web-heavy sub-agents have a separate
=gptel-agent-max-web-tool-calls= budget; calls beyond it are likewise blocked
without ending the run.  Round limits still return an explicit inconclusive
report with the attempts made, reason and confidence.  Web searches default to
